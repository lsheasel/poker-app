...
const isBlockedUsername = (name) => {
  ...
  const handleSignUp = async (e) => {
    ...
    // Form validation
    if (!username.trim()) {
      setPopup({ open: true, message: "Please enter a username!", type: "error" });
      return;
    }
    ...
    if (!password) {
      setPopup({ open: true, message: "Please enter a password!", type: "error" });
      return;
    }
    
    if (password.length < 6) {
      setPopup({ open: true, message: "Password must be at least 6 characters long!", type: "error" });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setPopup({ open: true, message: result.error.message, type: "error" });
      }
    } catch (err) {
